...
import { CheckCircle, Check, Loader2, AlertCircle, LogOut, Star } from 'lucide-react'
import { Logo } from '@/components/ui/Logo'
import { cn } from '@/lib/utils'
import { planApi, paymentApi, type PlanOut, type PatientVerifyResponse } from '@/lib/api'
import { useAuth } from '@/context/AuthContext'

// ─── Step indicator ────────────────────────────────────────────────────────────
const STEPS = ['Account', 'Choose Plan', 'Payment', 'Dashboard'] as const
...
function PlanCard({ plan, selected, onSelect }: {
  ...
  const perMonth = plan.duration_months > 1
    ? Math.round(plan.price / plan.duration_months)
    : null

  const durationLabel = plan.duration_months === 0
    ? '7 DAYS'
  ...
  return (
    ...
    >
      ...
      <p className="text-[10px] font-body font-semibold text-[#6B7280] tracking-widest uppercase mb-2">
        {durationLabel}
      </p>

      <p className="font-display text-3xl text-[#1F2937] mb-0.5">
        ₹{plan.price.toLocaleString('en-IN')}
      </p>
      {perMonth && (
        <p className="text-xs text-[#9CA3AF] font-body mb-4">
        ...
        {plan.features.map((f) => (
          <li key={f} className="flex items-start gap-2 text-xs text-[#4B5563] font-body">
            <Check className={cn(
              ...
              selected ? 'text-[#1F2937]' : 'text-[#9CA3AF]',
            )} />
            {f}
          </li>
        ))}
...
function SelectPlanInner() {
  ...
  // patient_id from URL (set by register redirect) or from auth context
  const patientId = searchParams.get('pid') ?? user?.id ?? undefined

  const [plans, setPlans] = useState<PlanOut[]>([])
  const [loading, setLoading] = useState(true)
  const [selected, setSelected] = useState<PlanOut | null>(null)

  const [step, setStep] = useState<'choose' | 'pay' | 'done'>(isRenewal ? 'choose' : 'choose')
  const [orderLoading, setOrderLoading] = useState(false)
  const [paying, setPaying] = useState(false)
  const [error, setError] = useState<string | null>(null)
  const [result, setResult] = useState<PatientVerifyResponse | null>(null)

  useEffect(() => {
    planApi.getPublicPlans('patient')
      .then((ps) => {
        setPlans(ps)
        setSelected(ps.find((p) => p.is_popular) ?? ps[0] ?? null)
      })
      .catch(() => setError('Could not load plans. Please refresh the page.'))
  ...
  async function handleContinue() {
    ...
    if (!window.Razorpay) { setError('Payment SDK not loaded. Please refresh.'); return }

    setError(null)
    setOrderLoading(true)
    let orderId: string
    try {
      const res = await paymentApi.patientCreateOrder(selected.id, selected.price * 100, patientId)
    ...
    const firstName = user ? (user as { first_name?: string }).first_name ?? user.email : 'there'
    ...
    new window.Razorpay({
      ...
      handler: async (response) => {
        setPaying(true)
        try {
          const res = await paymentApi.patientVerifyPayment({
  ...
  const firstName = user ? (user as { first_name?: string }).first_name ?? '' : ''
      ...
      <div className="min-h-screen bg-[#F9F7F4] flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="text-center space-y-5 max-w-sm">
            ...
            <h2 className="font-display text-3xl text-[#1F2937]">You&apos;re all set!</h2>
            <p className="text-sm text-[#6B7280] font-body">
              Your <span className="font-semibold text-[#1F2937]">{selected?.name}</span> subscription
              is active until{' '}
    ...
    <div className="min-h-screen bg-[#F9F7F4] flex flex-col">
      ...
      <header className="bg-[#1A1A1A] px-6 py-4 flex items-center justify-between">
        <span className="font-display text-xl text-white">
          Glimmora<span className="text-[#B8860B] italic">Care</span>
        </span>
        <Steps current={currentStep} />
        <button
          onClick={logout}
          className="flex items-center gap-1.5 text-xs text-white/50 hover:text-white/80 font-body transition-colors"
        >
      ...
      <div className="bg-[#1A1A1A] pt-10 pb-14 text-center px-4">
        {firstName && (
          <p className="text-[10px] font-body uppercase tracking-[0.2em] text-white/50 mb-3">
            Welcome, {firstName}
          </p>
        )}
        ...
        <p className="text-sm text-white/50 font-body max-w-md mx-auto">
          Unlock AI health insights, encrypted records, and family health management.
        </p>
      </div>
      ...
      <div className="flex-1 px-4 py-10 max-w-6xl mx-auto w-full">
        {loading ? (
          ...
          <div className="flex flex-col items-center py-20 gap-3">
            ...
            <p className="text-sm font-body text-[#6B7280]">No subscription plans available. Please contact support.</p>
          </div>
        ) : (
          <div className={cn(
            'grid gap-5',
        ...
        <div className="mt-10 flex flex-col items-center gap-2">
          <button
            ...
            className={cn(
              ...
              selected && !orderLoading && !paying
                ? 'bg-[#1F2937] text-white hover:bg-[#111827] shadow-md hover:shadow-lg'
          ...
          {!selected && (
            <p className="text-xs text-[#9CA3AF] font-body">Select a plan above to continue</p>
          )}
          {selected && !orderLoading && !paying && (
